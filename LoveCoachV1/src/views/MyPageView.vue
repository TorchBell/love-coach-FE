...
<template>
  <MainLayout>
    <div class="p-6 md:p-10 max-w-6xl mx-auto space-y-12 animate-fade-in-up">
      ...
      <div class="flex flex-col md:flex-row gap-8 items-center md:items-start bg-white/60 backdrop-blur-md p-8 rounded-3xl shadow-lg border border-white/50">
        <!-- Profile -->
        ...
        <div class="flex-1 w-full grid grid-cols-3 gap-4">
          <div class="bg-white/80 p-4 rounded-2xl shadow-sm border border-pastel-red/10 flex flex-col items-center justify-center gap-2 hover:scale-105 transition-transform">
            ...
            <span class="font-bold text-gray-600">토마 토큰</span>
          ...
          <div class="bg-white/80 p-4 rounded-2xl shadow-sm border border-pastel-blue/10 flex flex-col items-center justify-center gap-2 hover:scale-105 transition-transform">
            <span class="text-3xl">💪</span>
            <span class="font-bold text-gray-600">벨 토큰</span>
          ...
          <div class="bg-white/80 p-4 rounded-2xl shadow-sm border border-pastel-yellow/10 flex flex-col items-center justify-center gap-2 hover:scale-105 transition-transform">
            ...
            <span class="font-bold text-gray-600">치이 토큰</span>
      ...
      <div class="grid md:grid-cols-2 gap-8">
        ...
        <div class="bg-white/60 backdrop-blur-md p-8 rounded-3xl shadow-lg border border-white/50 space-y-6">
          <h3 class="text-xl font-bold text-gray-700 flex items-center gap-2">
            <span>📊</span> 활동 요약 (총 기록 일수)
          </h3>
          
          <div class="grid grid-cols-3 gap-4 text-center">
            <!-- Diet -->
            <div class="bg-white/50 p-4 rounded-2xl border border-pastel-green/20">
              <p class="text-sm font-bold text-gray-500 mb-2">식단</p>
              <p class="text-3xl font-bold text-pastel-green">{{ animatedStats.diet }}<span class="text-sm text-gray-400 ml-1">일</span></p>
            </div>

            <!-- Strength -->
            <div class="bg-white/50 p-4 rounded-2xl border border-pastel-blue/20">
              <p class="text-sm font-bold text-gray-500 mb-2">근력</p>
              <p class="text-3xl font-bold text-pastel-blue">{{ animatedStats.strength }}<span class="text-sm text-gray-400 ml-1">일</span></p>
            </div>

            <!-- Cardio -->
            <div class="bg-white/50 p-4 rounded-2xl border border-pastel-yellow/20">
              <p class="text-sm font-bold text-gray-500 mb-2">유산소</p>
              <p class="text-3xl font-bold text-pastel-yellow">{{ animatedStats.cardio }}<span class="text-sm text-gray-400 ml-1">일</span></p>
            </div>
          </div>
        </div>
          ...
          <div class="space-y-4">
            ...
            <div>
              <div class="flex justify-between mb-1">
                <span class="font-bold text-gray-600">토마 (Toma)</span>
                <span class="font-bold text-pastel-red">{{ animatedAffinity.toma }}%</span>
              </div>
              <div class="w-full bg-gray-200 rounded-full h-4 overflow-hidden">
                <div class="bg-pastel-red h-4 rounded-full transition-all duration-300" :style="{ width: `${animatedAffinity.toma}%` }"></div>
              </div>
            </div>
            ...
            <div>
              <div class="flex justify-between mb-1">
                <span class="font-bold text-gray-600">벨 (Belle)</span>
                <span class="font-bold text-pastel-blue">{{ animatedAffinity.belle }}%</span>
              </div>
              <div class="w-full bg-gray-200 rounded-full h-4 overflow-hidden">
                <div class="bg-pastel-blue h-4 rounded-full transition-all duration-300" :style="{ width: `${animatedAffinity.belle}%` }"></div>
              </div>
            </div>
            ...
            <div>
              <div class="flex justify-between mb-1">
                <span class="font-bold text-gray-600">치이 (Chie)</span>
                <span class="font-bold text-pastel-yellow">{{ animatedAffinity.chie }}%</span>
              </div>
              <div class="w-full bg-gray-200 rounded-full h-4 overflow-hidden">
                <div class="bg-pastel-yellow h-4 rounded-full transition-all duration-300" :style="{ width: `${animatedAffinity.chie}%` }"></div>
              </div>
            </div>
          </div>
        </div>
